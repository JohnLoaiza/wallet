...
class DbConn {
  ...
  Future initDB() async {
    ...
    database = await openDatabase(
      join(databasesPath, 'incomess2.db'),
      onCreate: (db, version) {
        return db.execute(
          "CREATE TABLE trans(id INTEGER PRIMARY KEY, date TEXT, name TEXT, type TEXT,nombre TEXT,telefono TEXT unique, amount INTEGER)",
        );
      },
      version: 1,
    );
  ...
  Future<List<Trans>> trans() async {
    ...
    return List.generate(maps.length, (i) {
      return Trans(
        ...
        nombreCli: maps[i]['nombre'],
        telefonoCli: maps[i]['telefono'],
